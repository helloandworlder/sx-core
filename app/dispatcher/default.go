...
func (d *DefaultDispatcher) getLink(ctx context.Context) (*transport.Link, *transport.Link) {
	...
	if user != nil && len(user.Email) > 0 {
		...
		// sx-core: apply per-user bandwidth rate limiting
		if ul := ratelimit.Manager.Get(user.Email); ul != nil {
			if ul.Ingress != nil {
				// Rate limit data flowing to user (downlink = outboundLink.Writer)
				origWriter := outboundLink.Writer
				outboundLink.Writer = &rateLimitStatWriter{
					Writer:  origWriter,
					bucket:  ul.Ingress,
					tracker: ul.TrackIngress,
				}
			}
			if ul.Egress != nil {
				// Rate limit data flowing from user (uplink = inboundLink.Writer)
				origWriter := inboundLink.Writer
				inboundLink.Writer = &rateLimitStatWriter{
					Writer:  origWriter,
					bucket:  ul.Egress,
					tracker: ul.TrackEgress,
				}
			}
		}
	}
...
func (d *DefaultDispatcher) DispatchLink(ctx context.Context, destination net.Destination, outbound *transport.Link) error {
	...
	if sessionInbound != nil && sessionInbound.User != nil && len(sessionInbound.User.Email) > 0 {
		if ul := ratelimit.Manager.Get(sessionInbound.User.Email); ul != nil {
			if ul.Ingress != nil {
				origWriter := outbound.Writer
				outbound.Writer = &rateLimitStatWriter{
					Writer:  origWriter,
					bucket:  ul.Ingress,
					tracker: ul.TrackIngress,
				}
			...
			if ul.Egress != nil {
				origReader := outbound.Reader
				outbound.Reader = &rateLimitStatReader{
					Reader:  origReader,
					bucket:  ul.Egress,
					tracker: ul.TrackEgress,
				}
