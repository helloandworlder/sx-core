...
func (v *SocksServerConfig) Build() (proto.Message, error) {
	...
	if len(v.Accounts) > 0 {
		...
		config.AccountEmails = make(map[string]string, len(v.Accounts))
		for _, account := range v.Accounts {
			config.Accounts[account.Username] = account.Password
			if account.Email != "" {
				config.AccountEmails[account.Username] = account.Email
			}
		}
		if len(config.AccountEmails) == 0 {
			config.AccountEmails = nil
		}
	}
